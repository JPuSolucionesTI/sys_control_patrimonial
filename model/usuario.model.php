...
where usuario.eliminado=0 order by idUsuario desc" );
    ...
    public function Consultar_informacion_usuario($idUsuario)
    {
        ...
        $this->bd = new Conexion();
        $stmt = $this->bd->prepare("SELECT idUsuario,idPersona,dni,primer_nombre,segundo_nombre,apellido_paterno,apellido_materno,fecha_nacimiento,sexo,correo,perfil.idPerfil,perfil.nombre as perfil,persona.codigo from usuario
inner join perfil on perfil.idPerfil=usuario.Perfil_id
 ...
 where idUsuario=$idUsuario;" );
	...
	public function Validar_Usuario(Usuario $usuario)
    {
        ...
        {       
            ...
            $stmt = $this->bd->prepare("SELECT * FROM usuario WHERE Correo=:Correo and Estado=1");
            $stmt->bindValue(':Correo', $usuario->__GET('Correo'));
            ...
            if($stmt->rowCount() > 0)
            {
                ...
                if(strtolower($usuario->__GET('Clave'))==strtolower($usuario_registrado['Clave']))
                {
